...
MyHarness는 브라우저 연결이 끊겨도 런처와 backend session을 유지합니다.

- 브라우저/EventSource 연결이 닫혀도 관련 backend를 자동 종료하지 않습니다.
- 모든 backend session이 사라져도 web server는 계속 실행되며, 다시 접속해서 새 session을 시작할 수 있습니다.
