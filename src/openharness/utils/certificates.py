...
POSCO_CERT_FILE = Path("C:/POSCO.crt")
_BUNDLE_FILE_NAME = "posco-ca-bundle.pem"
_PYTHON_CA_ENV_VARS = ("SSL_CERT_FILE", "REQUESTS_CA_BUNDLE", "CURL_CA_BUNDLE")
...
def configure_posco_certificate() -> str | None:
    """Configure process-wide CA bundle settings when C:/POSCO.crt exists."""
    return _configure_certificate(POSCO_CERT_FILE)


def httpx_verify_argument() -> bool | str:
    """Return the verify value to use for httpx clients that ignore env vars."""
    if _configured_bundle:
        return _configured_bundle
    return configure_posco_certificate() or True
