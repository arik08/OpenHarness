"""Settings model and loading logic for MyHarness.

Settings are resolved with the following precedence (highest first):
1. CLI arguments
2. Environment variables (ANTHROPIC_API_KEY, MYHARNESS_MODEL, etc.)
3. Config file (~/.myharness/settings.json)
4. Defaults
"""
...
class DockerSandboxSettings(BaseModel):
    ...
    image: str = "myharness-sandbox:latest"
...
class Settings(BaseModel):
    """Main settings model for MyHarness."""
...
def _apply_env_overrides(settings: Settings) -> Settings:
    ...
    explicitly configure the corresponding field.  ``MYHARNESS_*`` env vars
    ...
    # --- base_url ---
    myharness_base = os.environ.get("MYHARNESS_BASE_URL")
    if myharness_base:
        updates["base_url"] = myharness_base
    ...
    max_turns = os.environ.get("MYHARNESS_MAX_TURNS")
    if max_turns:
        updates["max_turns"] = int(max_turns)

    context_window_tokens = os.environ.get("MYHARNESS_CONTEXT_WINDOW_TOKENS")
    if context_window_tokens:
        updates["context_window_tokens"] = int(context_window_tokens)

    auto_compact_threshold_tokens = os.environ.get("MYHARNESS_AUTO_COMPACT_THRESHOLD_TOKENS")
    if auto_compact_threshold_tokens:
        updates["auto_compact_threshold_tokens"] = int(auto_compact_threshold_tokens)

    api_format = os.environ.get("MYHARNESS_API_FORMAT")
    if api_format:
        updates["api_format"] = api_format

    provider = os.environ.get("MYHARNESS_PROVIDER")
    if provider:
        updates["provider"] = provider

    shell = os.environ.get("MYHARNESS_SHELL")
    if shell:
        updates["shell"] = shell

    sandbox_enabled = os.environ.get("MYHARNESS_SANDBOX_ENABLED")
    sandbox_fail = os.environ.get("MYHARNESS_SANDBOX_FAIL_IF_UNAVAILABLE")
    sandbox_backend = os.environ.get("MYHARNESS_SANDBOX_BACKEND")
    sandbox_docker_image = os.environ.get("MYHARNESS_SANDBOX_DOCKER_IMAGE")
...
def load_settings(config_path: Path | None = None) -> Settings:
    ...
    if config_path is None:
        from myharness.config.paths import get_config_file_path
