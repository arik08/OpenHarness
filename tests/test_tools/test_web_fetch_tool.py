...
from myharness.tools.web_fetch_tool import WebFetchTool, WebFetchToolInput, _html_to_text
from myharness.tools.web_search_tool import WebSearchTool, WebSearchToolInput
...
async def test_web_fetch_tool_reads_html(tmp_path, monkeypatch):
    ...
    async def fake_fetch(url: str, **_: object) -> httpx.Response:
        ...
        return httpx.Response(
            ...
            text="<html><body><h1>MyHarness Test</h1><p>web fetch works</p></body></html>",
            request=request,
        )
...
async def test_web_search_tool_reads_results(tmp_path, monkeypatch):
    ...
    async def fake_fetch(url: str, **kwargs: object) -> httpx.Response:
        ...
        body = (
            "<html><body>"
            '<a class="result__a" href="https://example.com/docs">MyHarness Docs</a>'
