...
def test_build_backend_command_includes_flags():
    ...
    assert command[:3] == [command[0], "-m", "myharness"]
...
async def test_run_print_mode_passes_cwd_to_build_runtime(monkeypatch):
    ...
    monkeypatch.setattr("myharness.ui.app.build_runtime", _build_runtime)
    monkeypatch.setattr("myharness.ui.app.start_runtime", _start_runtime)
...
async def test_run_task_worker_reads_one_shot_json_line(monkeypatch):
    ...
    monkeypatch.setattr("myharness.ui.app.handle_line", _handle_line)
    monkeypatch.setattr("myharness.ui.app.close_runtime", _close_runtime)
    monkeypatch.setattr("myharness.ui.app.sys.stdin", _FakeStdin())
...
async def test_run_print_mode_waits_for_coordinator_async_agents(monkeypatch):
    ...
    monkeypatch.setattr("myharness.ui.app.asyncio.sleep", _sleep)
