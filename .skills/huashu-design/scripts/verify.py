#!/usr/bin/env python3
"""
verify.py — Playwright wrapper for verifying generated HTML

Usage:
    python verify.py path/to/design.html                    # basic: open, screenshot, and capture console errors
    python verify.py design.html --viewports 1920x1080,375x667  # multiple viewports
    python verify.py deck.html --slides 10                  # capture slides one by one（10）
    python verify.py design.html --output ./screenshots/   # output directory
    python verify.py design.html --show                    # non-headless; open a real browser window
...
def verify_html(html_path, viewports=None, slides=0, output_dir=None, show=False, wait=2000):
    ...
    if console_errors:
        ...
        if len(console_errors) > 20:
            print(f"  ... more{len(console_errors) - 20}items")
    else:
...
def main():
    ...
    parser.add_argument("--slides", type=int, default=0,
                        help="Slide mode: capture the first N slides; HTML must support ArrowRight navigation")
    parser.add_argument("--output", default=None,
                        help="output directory（HTMLscreenshots/）")
